(function(root){'use strict';
 ...
 function step(s,inputs,dt){dt=clamp(dt,0,1/30);if(s.winner>=0)return;s.time+=dt;if(s.wait>0){s.wait=Math.max(0,s.wait-dt);return;}for(const p of s.pads)p.ready=Math.max(0,p.ready-dt);
  for(const c of s.cars){const k=inputs[c.index]||{},forward=(k.up?1:0)-(k.down?1:0),steer=(k.right?1:0)-(k.left?1:0);const ground=surface(c.x,c.z),grounded=c.y<=ground.y+.5&&c.vy<=1;
   ...
   if(grounded){c.angle+=steer*2.5*dt*(forward<0?-1:1);const speed=forward*(boost?350:230),blend=1-Math.exp(-4*dt);c.vx+=(Math.cos(c.angle)*speed-c.vx)*blend;c.vz+=(Math.sin(c.angle)*speed-c.vz)*blend;}
   else{const ease=1-Math.exp(-5*dt);c.angle+=steer*.85*dt;c.pitch+=(forward*.5-(c.pitch||0))*ease;c.roll+=(steer*.22-(c.roll||0))*ease;if(boost){c.vx+=Math.cos(c.angle)*Math.cos(c.pitch)*230*dt;c.vz+=Math.sin(c.angle)*Math.cos(c.pitch)*230*dt;c.vy+=Math.sin(c.pitch)*360*dt;}}
   const jumpPressed=k.jump&&!c.jumpHeld;
   if(jumpPressed&&(grounded||c.jumps<2)){c.jumps++;if(c.jumps===1){c.vy=340; c.y=ground.y+1;}else{c.vy=Math.max(c.vy,160);if(forward||steer){const a=c.angle+Math.atan2(steer,forward),dodgeSpeed=clamp(Math.hypot(c.vx,c.vz)*.75+160,270,420);c.vx=Math.cos(a)*dodgeSpeed;c.vz=Math.sin(a)*dodgeSpeed;c.dodge=.55;c.dodgeAge=0;c.dodgeForward=forward;c.dodgeSide=steer;}}}
   ...
   if(c.y>C.ceiling-55){c.y=C.ceiling-55;c.vy=Math.min(0,c.vy);}for(const v of ['vx','vz'])c[v]=clamp(c[v],-520,520);
   contain(c,24,0);
  ...
  const b=s.ball;b.vy-=315*dt;b.x+=b.vx*dt;b.y+=b.vy*dt;b.z+=b.vz*dt;const drag=Math.exp(-.24*dt);b.vx*=drag;b.vz*=drag;
  for(const c of s.cars){const dx=b.x-c.x,dy=b.y-(c.y+20),dz=b.z-c.z,d=Math.hypot(dx,dy,dz),radius=51;if(d<radius){const nx=d>0?dx/d:Math.cos(c.angle),ny=d>0?dy/d:0,nz=d>0?dz/d:Math.sin(c.angle);b.x=c.x+nx*radius;b.y=c.y+20+ny*radius;b.z=c.z+nz*radius;const relative=(c.vx-b.vx)*nx+(c.vy-b.vy)*ny+(c.vz-b.vz)*nz;const impulse=Math.max(25,relative*1.72+34+(c.dodge>0?132:0));b.vx+=nx*impulse;b.vy+=Math.max(0,ny*impulse)+Math.max(0,c.vy)*.45;b.vz+=nz*impulse;s.hitId++;s.lastTouch=c.index;s.hitPoint={x:b.x,y:b.y,z:b.z};}}
  ...
  const floor=surface(b.x,b.z);if(b.y<floor.y+C.ballRadius){b.y=floor.y+C.ballRadius;b.vx-=floor.nx*floor.slope*180*dt;b.vz-=floor.nz*floor.slope*180*dt;b.vy=Math.abs(b.vy)>30?Math.abs(b.vy)*.62:0;b.vx*=Math.exp(-.65*dt);b.vz*=Math.exp(-.65*dt);}for(const v of ['vx','vy','vz'])b[v]=clamp(b[v],-650,650);
 }
